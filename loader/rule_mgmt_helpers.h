...
int add_src_ip_vector(struct config *cfg, int value) {
...
int add_dst_ip_vector(struct config *cfg, int value) {
...
int add_sport_vector(struct config *cfg, int value) {
	int len;
	int map_fd;
	char map_path[PATH_MAX];
	struct rule_vector vector;;
...
int add_dport_vector(struct config *cfg, int value) {
	int len;
	int map_fd;
	char map_path[PATH_MAX];
	struct rule_vector vector;;
...
int add_dev_vector(struct config *cfg, int value) {
	int len;
	int map_fd;
	char map_path[PATH_MAX];
	struct rule_vector vector;;
...
int add_rule(struct config *cfg, int isPolicy)
{
	...
	if (isPolicy)
		err = add_vectors(cfg, 0);
	else
		err = add_vectors(cfg, 1);
